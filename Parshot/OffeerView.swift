...
class OffeerView: BaseCell {
    ...
    override func setupViews() {
        ...
        ///////// old price label
        mainView.addSubview(oldPriceLabe)
        oldPriceLabe.topAnchor.constraint(equalTo: nameLabe.bottomAnchor,constant : 5).isActive = true
        oldPriceLabe.rightAnchor.constraint(equalTo:mainView.rightAnchor,constant :-20).isActive = true
    
        mainView.addSubview(oldPriceValueLabe)
        oldPriceValueLabe.topAnchor.constraint(equalTo: nameLabe.bottomAnchor,constant : 5).isActive = true
        oldPriceValueLabe.leftAnchor.constraint(equalTo:oldPriceLabe.rightAnchor).isActive = true
    
    
        ////// new price label
        mainView.addSubview(newPriceLabe)
        newPriceLabe.topAnchor.constraint(equalTo: nameLabe.bottomAnchor,constant : 5).isActive = true
        newPriceLabe.leftAnchor.constraint(equalTo:mainView.leftAnchor,constant :20).isActive = true
        
        mainView.addSubview(newpriceValueLabe)
        newpriceValueLabe.topAnchor.constraint(equalTo: nameLabe.bottomAnchor,constant : 5).isActive = true
        newpriceValueLabe.leftAnchor.constraint(equalTo:newPriceLabe.rightAnchor).isActive = true
    ...
    let oldPriceValueLabe :UILabel = {
        let NL = UILabel()
        NL.textColor = UIColor.black
        var font = UIFont(name: "JFFlat-Regular", size: 10)
        NL.font = font
        NL.translatesAutoresizingMaskIntoConstraints = false
        NL.text = NSLocalizedString("40", comment: "this is name")
    ...
    let newpriceValueLabe :UILabel = {
        let NL = UILabel()
        NL.textColor = UIColor.black
        var font = UIFont(name: "JFFlat-Regular", size: 10)
        NL.font = font
        NL.translatesAutoresizingMaskIntoConstraints = false
        NL.text = NSLocalizedString("30", comment: "this is name")
