...
class ParashotDealsController: BaseCell,FloatRatingViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: self.cell, for: indexPath) as! ParashotDealsView
    ...
    override func setupViews() {
        ...
        collectionView.register(ParashotDealsView.self, forCellWithReuseIdentifier: cell)
        ...
        itemName.topAnchor.constraint(equalTo: horizantalLine.bottomAnchor, constant : 30 ).isActive = true
        ...
        mainView.addSubview(describeLabel)
        describeLabel.topAnchor.constraint(equalTo: itemRateNum.bottomAnchor,constant:10).isActive = true
        describeLabel.rightAnchor.constraint(equalTo: mainView.rightAnchor,constant: -10).isActive = true
         describeLabel.widthAnchor.constraint(equalTo: mainView.widthAnchor,multiplier : 0.8).isActive = true
         describeLabel.textAlignment = .right
        
        //////////// price value
        mainView.addSubview (oldVluePrice)
        oldVluePrice.topAnchor.constraint(equalTo: horizantalLine.bottomAnchor,constant : 10).isActive = true
        oldVluePrice.leftAnchor.constraint(equalTo: leftAnchor,constant : 30 ).isActive = true
        ////// price word
        mainView.addSubview(oldPrice)
        oldPrice.topAnchor.constraint(equalTo: horizantalLine.bottomAnchor,constant : 10 ).isActive = true
        oldPrice.rightAnchor.constraint(equalTo: oldVluePrice.leftAnchor ,constant : -5).isActive = true
        /////
        mainView.addSubview (newValuePrice)
        newValuePrice.topAnchor.constraint(equalTo: oldVluePrice.bottomAnchor,constant : 10).isActive = true
        newValuePrice.leftAnchor.constraint(equalTo: leftAnchor,constant : 30 ).isActive = true
        ////// price word
        mainView.addSubview(newPrice)
        newPrice.topAnchor.constraint(equalTo: oldVluePrice.bottomAnchor,constant : 10 ).isActive = true
        newPrice.rightAnchor.constraint(equalTo: newValuePrice.leftAnchor ,constant : -5).isActive = true
        //////// charge btn
        mainView.addSubview(itemChargeBtn)
        itemChargeBtn.topAnchor.constraint(equalTo: newPrice.bottomAnchor,constant : 5 ).isActive = true
   ...
   ///////
    ...
    let describeLabel :UILabel = {
        ...
        NL.font = font
        NL.translatesAutoresizingMaskIntoConstraints = false
        NL.text = NSLocalizedString("خصم ٢٠٪ في حالة شراء ٣ قطع من هذا المنتج", comment: "this is name")
        return NL
